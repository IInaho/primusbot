...
func (m *Model) handleProcessingKey(msg tea.KeyPressMsg) tea.Cmd {
	...
	case "enter":
		...
		if value != "" {
			...
			_ = m.Runtime.Steer(context.Background(), "", controlruntime.Input{
	...
	case "esc":
		_ = m.Runtime.Abort(context.Background(), "")
...
func (m *Model) refreshSuggestions() {
	m.Suggestions.Refresh(m.Input.Value(), m.Runtime.CommandNames())
	m.resizeMessages()
}
...
func (m *Model) handleRuntimeEvent(ev controlruntime.Event) tea.Cmd {
	...
	case controlruntime.EventSystemMessage:
		...
		prov, mod := m.Runtime.ProviderModel()
	...
	case controlruntime.EventApprovalRequested:
		if p, ok := ev.Payload.(controlruntime.ApprovalView); ok {
			...
			m.ConfirmBar.SetRequestWithResponder(&req, func(ok, remember bool) {
				_ = m.Runtime.Approve(context.Background(), p.ID, controlruntime.ApprovalDecision{
					Allowed:  ok,
	...
	case controlruntime.EventRunDone:
		...
		return m.handleDone(doneMsg{
			...
			duration: m.Runtime.Stats().Duration,
			tokens:   tokensSummary(m.Runtime.Stats()),
		})
	...
	case controlruntime.EventRunAborted:
		return m.handleDone(doneMsg{err: errors.New("aborted")})
	case controlruntime.EventSessionResumed:
