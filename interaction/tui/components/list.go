// List 通用滚动列表组件：管理 Item 集合、视口滚动、鼠标滚轮支持。
package components
...
// InsertItem inserts item at index idx without resetting the scroll offset.
// Cached renders of shifted items are preserved; the scroll offset is adjusted
// only when the insertion happens at or above the current viewport top.
func (l *List) InsertItem(idx int, item Item) {
	...
	l.cacheMu.Lock()
	shifted := make(map[int]renderedItem, len(l.cache))
	for i, ri := range l.cache {
		if i >= idx {
			shifted[i+1] = ri
		} else {
			shifted[i] = ri
