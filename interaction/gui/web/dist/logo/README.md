...
品牌标识源码与多格式导出。本目录位于 `interaction/gui/web/public/`，所有资源均为 GUI 专属。
...
| `logo-icon-app.svg` | GUI 浏览器标签 favicon（引用自 `interaction/gui/web/index.html`） |
...
```bash
cd interaction/gui/web/public/logo
# 1. 多尺寸 PNG
...
node build_icns.js
# 4. 同步到项目
cp appicon.icns appicon.ico ../../../app/
