...
func TestApplyEmptyCWD(t *testing.T) {
	target := &fakeTarget{}
	got := Apply(target, ApplyOptions{ContextWindow: 100})
...
func TestApplyAddsProjectText(t *testing.T) {
	target := &fakeTarget{}
	got := Apply(target, ApplyOptions{
		CWD:           "/repo",
		ContextWindow: 200,
