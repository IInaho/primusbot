...
func TestIndexAll(t *testing.T) {
	...
	dbPath := filepath.Join(dir, ".nekocode", "index.db")
...
func TestLoadOrBuildRebuildsStaleIndex(t *testing.T) {
	dir := setupTestProject(t)
	...
	if _, err := indexer.LoadOrBuild(dir); err != nil {
		t.Fatalf("LoadOrBuild initial: %v", err)
	}
	...
	updated := string(data) + "\nfunc AddedAfterIndex() {}\n"
	if err := os.WriteFile(mainPath, []byte(updated), 0644); err != nil {
		t.Fatal(err)
	}

	g, err := indexer.LoadOrBuild(dir)
	if err != nil {
		t.Fatalf("LoadOrBuild stale: %v", err)
	}
	if got := g.QuerySymbol("AddedAfterIndex"); len(got) != 1 {
		t.Fatalf("stale index was not rebuilt, got %d symbols", len(got))
	}
}
