...
- **持久化** — SQLite WAL 模式，索引结果缓存到 `.nekocode/index.db`（纯 Go 驱动，无 CGO）
...
Index 在项目启动时一次性构建**代码知识图谱**，将 O(n) 的文件扫描转化为 O(1) 的内存查找。

### Tree-sitter 解析

[Tree-sitter](https://tree-sitter.github.io/) 是增量解析器，能将源代码解析为具体的语法树（CST）。Index 用 tree-sitter query 精确提取符号定义和调用关系：
...
└── IndexTool.Execute(query)
    ...
    │
    └── "search:term"   → indexer.SearchFTS(term)
...
bot/extension/tool/builtin/index/
...
import (
    ...
    indextool "nekocode/bot/extension/tool/builtin/index"
)
