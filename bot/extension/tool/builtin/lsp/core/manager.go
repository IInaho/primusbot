...
// NewManager creates a manager rooted at wsRoot. Specs may be built with
// DefaultSpecs and overridden per language.
...
// Close shuts down every spawned language server and cancels the manager's
// session context. Idempotent.
...
// DefaultSpecs maps a language key to its conventional server. Commands are
// tried on PATH; nothing here ships with nekocode. Extensions drive file
// routing, so a caller can override any entry or add a new language entirely.
...
// notInstalledError carries the install hint so a tool can tell the model
// exactly how to make the capability available.
...
func (m *Manager) spawn(spec ServerSpec) (*client, error) {
...
// Definition resolves where a symbol on the given line is defined.
...
// indexingOr turns a persistent ContentModified into a retry-shortly message
// the model can act on, leaving any other error to surface.
...
// References lists every reference to a symbol across the workspace. It waits
// for a freshly started server's background index to settle so cross-file call
// sites are not missed (see client.referencesSettled).
func (m *Manager) References(ctx context.Context, file string, line int, symbol string) (string, error) {
	c, uri, pos, err := m.prepare(ctx, file, line, symbol)
	if err != nil {
		return "", err
	}
	locs, err := c.referencesSettled(ctx, uri, pos, m.abs(file))
	if err != nil {
		return indexingOr(err)
	}
	return m.formatLocations("reference", locs), nil
}
...
// Diagnostics reports compiler/linter problems for a file, waiting briefly for
// the server to publish a fresh set after didOpen/didChange.
func (m *Manager) Diagnostics(ctx context.Context, file string) (string, error) {
	...
	diags := c.waitDiagnostics(ctx, uri, c.docVersion(uri), 3*time.Second)
	return formatDiagnostics(m.rel(path), path, diags), nil
}
