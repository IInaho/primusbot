...
func (t *ShellTool) Description() string {
	return fmt.Sprintf("Execute shell commands in a session-based sandbox. "+
		"Default action is run. A run waits up to yield_time_ms (default %dms); if the command is still running, it returns session_id and the process continues until timeout_ms (default %dms, max %dms). "+
		...
		defaultShellYield.Milliseconds(), defaultShellTimeout.Milliseconds(), maxShellTimeout.Milliseconds())
}
