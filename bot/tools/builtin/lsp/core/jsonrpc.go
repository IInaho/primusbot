// Package lspcore is a minimal Language Server Protocol client: it spawns a
// language server per language on demand, syncs queried documents from disk,
// and exposes a few read-only capabilities (definition, references, hover,
// diagnostics). Servers are not bundled — they resolve on PATH and a clear
...
// (document symbols, semantic tokens for a huge file) while stopping a corrupt
// or desynced Content-Length from triggering an unbounded allocation that OOMs
// the whole process.
const maxFrameBytes = 64 << 20 // 64 MiB

// readFrame reads one LSP message: header lines terminated by a blank line,
