...
func TestQuotaHook(t *testing.T) {
	...
	s.Set(hooks.StoreQuotaReads, 5)
	if r := hk.On(s); r != nil {
		t.Fatal("reads=5 should be silent")
	}
	s.Set(hooks.StoreQuotaReads, 2)
...
func TestVerificationHook(t *testing.T) {
	...
	s.Set(hooks.StoreHasTasks, 0)
	if r := hk.On(s); r != nil {
		t.Fatal("no tasks should be silent")
	}
	s.Set(hooks.StoreHasTasks, 1)
	s.Set(hooks.StoreTasksAllDone, 1)
	if r := hk.On(s); r != nil {
		t.Fatal("all tasks done should be silent")
	}
	s.Set(hooks.StoreTasksAllDone, 0)
...
func TestVerificationHookUsesStructuredFinalIntent(t *testing.T) {
	...
	s.Set(hooks.StoreHasTasks, 1)
	s.Set(hooks.StoreTasksAllDone, 0)
	s.Set(hooks.StoreTurnToolCalls, 0)

	s.SetStr(hooks.StoreFinalIntent, hooks.FinalIntentFormatError)
	if r := hk.On(s); r != nil {
		t.Fatalf("format error intent should be handled by garbled circuit breaker, got %+v", r)
	}

	s.SetStr(hooks.StoreFinalIntent, hooks.FinalIntentNonFinal)
	if r := hk.On(s); r != nil {
		t.Fatalf("non-final intent should be silent, got %+v", r)
	}

	s.SetStr(hooks.StoreFinalIntent, hooks.FinalIntentFinal)
...
func TestGarbledCircuitBreaker(t *testing.T) {
	...
	s.Set(hooks.StoreRespGarbled, 4)
	if r := hk.On(s); r != nil {
		t.Fatal("count=4 should not stop")
	}
	s.Set(hooks.StoreRespGarbled, 5)
	if r := hk.On(s); r == nil || r.Stop == nil || *r.Stop != hooks.StopFormatError {
...
func TestProgressStallHook(t *testing.T) {
	...
	s := newState()
	s.SetStr(hooks.StoreStepInput, "test task")
	s.Set(hooks.StoreTurnToolCalls, 1)
...
func TestExploreCascadeHook(t *testing.T) {
	...
	s.Set(hooks.StoreToolResearcher, 3)
	if r := hk.On(s); r != nil {
		t.Fatal("3 researchers should be silent")
	}
	s.Set(hooks.StoreToolResearcher, 4)
...
func TestToolResultGuardrailHook(t *testing.T) {
	...
	s.Set(hooks.StoreToolResultCount, 40)
	if r := hk.On(s); r != nil {
		t.Fatalf("threshold should be silent, got %+v", r)
	}
	s.Set(hooks.StoreToolResultCount, 41)
	if r := hk.On(s); r == nil || r.Hint == nil || r.Hint.Type != "tool_results" {
		t.Fatalf("tool result guardrail = %+v, want hint", r)
	}
	if s.Get(hooks.CounterToolResultWarned) != 41 {
		t.Fatalf("last warned = %d, want 41", s.Get(hooks.CounterToolResultWarned))
	}
	s.Set(hooks.StoreToolResultCount, 45)
	if r := hk.On(s); r != nil {
...
func TestReadBeforeWriteHook(t *testing.T) {
	...
	s := newState()
	s.SetTool("edit", nil)
	s.SetStr(hooks.StoreEditTargetPath, "main.go")
	s.Set(hooks.StoreEditTargetExists, 1)
	s.Set(hooks.StoreEditTargetWasRead, 0)
	s.Set(hooks.StoreEditAnchorSufficient, 0)

	if r := hk.On(s); r == nil || r.BlockTool == nil || !strings.Contains(r.BlockTool.Reason, "main.go") {
		t.Fatalf("unread edit result = %+v, want block", r)
	}

	s.Set(hooks.StoreEditTargetWasRead, 1)
	if r := hk.On(s); r != nil {
		t.Fatalf("read target should pass, got %+v", r)
	}

	s.Set(hooks.StoreEditTargetWasRead, 0)
	s.Set(hooks.StoreEditAnchorSufficient, 1)
	if r := hk.On(s); r != nil {
...
func TestReadOnlySpiralHook(t *testing.T) {
	...
	s.Set(hooks.StoreReadOnlyStreak, 2)
	if r := hk.On(s); r != nil {
		t.Fatalf("streak=2 should be silent, got %+v", r)
	}
	s.Set(hooks.StoreReadOnlyStreak, 3)
	if r := hk.On(s); r == nil || r.Hint == nil || r.Hint.Type != "read_only_spiral" {
		t.Fatalf("streak=3 result = %+v, want hint", r)
	}
	if s.Get(hooks.StoreReadOnlyStreak) != 0 {
		t.Fatalf("streak after hook = %d, want reset", s.Get(hooks.StoreReadOnlyStreak))
	}
}
