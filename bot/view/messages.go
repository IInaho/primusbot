	...
	for i < len(messages) {
		...
		case "user":
			if !isInternalMessage(m) {
				out = append(out, DisplayMessage{Role: "user", Content: m.Content})
			}
			i++
		case "assistant":
		...
		case "system":
			if !isInternalMessage(m) {
				out = append(out, DisplayMessage{Role: "system", Content: m.Content})
			}
			i++
		default:
			i++
...
func displayAssistantTurn(msgs []types.Message, idx int, toolNames, toolArgs map[string]string) (DisplayMessage, int) {
...
func appendDisplayToolResult(blocks []DisplayBlock, images []ImageRef, name, args string, msg types.Message) ([]DisplayBlock, []ImageRef) {
...
func extractImageRefs(output string) []ImageRef {
